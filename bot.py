...
def make_gspread():
    ...
    c = Credentials.from_service_account_info(d, scopes=scopes)
...
async def main():
    ...
    async def global_error_handler(event: ErrorEvent):
        log.error(f"Unhandled exception: {event.exception}")
        await notify_error("aiogram handler", event.exception)
        return True
    ...
    scheduler.add_job(heartbeat,      CronTrigger(hour="*/1", minute=30))
    scheduler.start()
